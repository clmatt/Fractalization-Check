...
int main() {
	...
	//Change this	
	const int n = 7;
	std::vector<std::vector<int> > num(13, std::vector<int>(13,0));
	...
	for(int i = n+1; i <= 10; ++i) { //CHANGE THIS
		std::cout << std::endl;
		//vector<Graph> newAllGraphs;
		
		for(int j = 0; j < allGraphs.size(); ++j) {
			std::cout << "(" << i << ", " << j << ") out of (10, " << allGraphs.size() << ")" << std::endl;
